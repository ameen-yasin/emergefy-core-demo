...
// import { Users, Star, Bell, X, Check, Play, Pause, SkipForward, Zap, Eye, EyeOff, Bot, Activity } from "lucide-react";
...
// function recommendBest(segments, offers) {
...
// function computePreviewMetrics(segments, offers, selectedSegment, selectedOffer) {
...
//   } catch (e) {
//     console.error("[demo-tests] Failure", e);
//   }
// }
...
// function PersonaCard({ reasoningVisible, setReasoningVisible, paused, setPaused }) {
//   return (
...
//     const id = Date.now();
//     const stepIdx = Math.floor(transcript.length / 4) % 1; // single round for demo
...
//   useEffect(() => { if (!paused && transcript.length < 4) { playSession(); } }, [paused]);
...
//         const optimized = Math.max(0, Math.min(50, Math.round(sim.filter((r) => r && r.type).length)));
...
//     try { if (typeof window === "undefined") return defaultKpi; const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const obj = JSON.parse(raw); return obj.kpi || defaultKpi; } } catch (e) {}
//     return defaultKpi;
//   });
//   useEffect(() => {
...
//           const obj = JSON.parse(raw);
//           const sim = Array.isArray(obj.simHistory) ? obj.simHistory : [];
//           const last24 = sim.slice(0, 7).map((s) => Math.max(1, Math.round((s.reactivated || 0) / 10)));
